...
//! - `--keep-last N`: 최신 백업 N벌을 보존(최신 체인부터 멤버 수를 누적해 N에 도달할 때까지).
//!   체인을 쪼개지 않으므로 살아있는 증분의 base가 단독 삭제되지 않는다.
//! - 여럿 주면 **합집합 보존**(어느 한 규칙이라도 보존하면 보존). 하나도 없으면 안전을 위해
//!   **아무것도 삭제하지 않는다**(명시적 기준 없는 삭제 금지). CLI 플래그가 없으면 config
//!   `[profiles.<name>.retention]`(keep_full/keep_days/keep_last)을 기본값으로 쓴다(CLI 우선).
//!
//! ## orphan / incomplete
